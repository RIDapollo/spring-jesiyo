...
<head>
    ...
    <style>
        ...
        /* 평점 별 애니메이션 */
        .star-rating i { transition: transform 0.3s ease; }
                        ...
                        <div class="flex flex-col md:flex-row gap-4 mb-8">
                            ...
                            <div class="flex items-center justify-center gap-2 bg-orange-50 px-5 py-3 rounded-2xl border border-orange-100">
                                ...
                                <span class="text-xl font-black text-[#FF8A3D]">
                                    <fmt:formatNumber value="${not empty member.point ? member.point : 0}" pattern="#,###" />
                                </span>
                                <span class="text-xs font-bold text-[#FF8A3D]">P</span>
                            </div>
                            
                            <%-- [변경됨] 평점(Score) 영역 --%>
                            <div class="flex flex-col justify-center px-5 py-3 bg-white border border-slate-100 rounded-2xl shadow-sm">
                                <div class="flex justify-between items-center mb-1 gap-4">
                                    <span class="text-[10px] font-black text-slate-400 tracking-tighter uppercase">판매자 평점 </span>
                                    <span class="text-sm font-black text-[#FF8A3D]">${member.avgScore} / 5.0</span>
                                </div>
                                <div class="flex gap-1 text-[#FF8A3D] star-rating">
                                    <c:forEach begin="1" end="5" var="i">
                                        <c:choose>
                                            <c:when test="${i <= member.avgScore}">
                                                <i class="fas fa-star text-sm"></i>
                                            </c:when>
                                            <c:when test="${i - 0.5 <= member.avgScore}">
                                                <i class="fas fa-star-half-alt text-sm"></i>
                                            </c:when>
                                            <c:otherwise>
                                                <i class="far fa-star text-sm text-slate-200"></i>
                                            </c:otherwise>
                                        </c:choose>
                                    </c:forEach>
                                </div>
