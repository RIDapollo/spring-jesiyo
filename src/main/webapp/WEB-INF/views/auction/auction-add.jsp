...
<body>
    ...
    <div class="page-wrap max-w-3xl">
        ...
        <form id="auctionForm" action="/jesiyo/auction" method="POST" enctype="multipart/form-data" class="content-card card-pad flex flex-col gap-10 bg-white border border-slate-200 rounded-2xl shadow-sm p-5 md:p-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2"> 중분류 <span class="text-rose-500">*</span></label>
                        <select id="subCategory" class="select select-bordered w-full focus:border-brand-500 focus:outline-none disabled:bg-slate-100" required disabled>
                    ...
                    <div>
                        ...
                        <select id="lastCategory" name="categorySeq" class="select select-bordered w-full focus:border-brand-500 focus:outline-none disabled:bg-slate-100" required disabled>
                            <option disabled selected value="">중분류를 먼저 선택하세요</option>
                        </select>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // [폼 제출 검증] 소분류까지 반드시 선택했는지 확인
        auctionForm.addEventListener('submit', function(e) {
            if (lastCategory.disabled || lastCategory.value === "") {
                e.preventDefault(); // 폼 제출 차단
                alert('카테고리를 소분류까지 모두 선택해주세요.');
                lastCategory.focus(); // 소분류로 포커스 이동
            }
        });
        ...
        // (2) 대분류 선택 시 중분류(Children) 가져오기
        mainCategory.addEventListener('change', function() {
            const parentSeq = this.value;

            // 중분류 초기화
            subCategory.innerHTML = '<option disabled selected value="">중분류 선택</option>';
            subCategory.disabled = true;

            // [핵심 변경점] 대분류가 바뀌면 소분류도 강제로 초기화 및 비활성화 처리
            lastCategory.innerHTML = '<option disabled selected value="">중분류를 먼저 선택하세요</option>';
            lastCategory.disabled = true;
            ...
            if (parentSeq) {
                ...
                fetch(url)
                    ...
                    .catch(error => console.error('중분류 로드 실패:', error));
            }
        });
        
        // (3) 중분류 선택 시 소분류(Children) 가져오기
        subCategory.addEventListener('change', function() {
            const parentSeq = this.value;

            // 소분류 초기화
            lastCategory.innerHTML = '<option disabled selected value="">소분류 선택</option>';
            lastCategory.disabled = true;
                    ...
                    .then(data => {
                        if(data.length > 0) {
                            data.forEach(category => {
                                ...
                                option.value = category.seq;
                                option.text = category.name;
                                lastCategory.appendChild(option);
                            });
                            lastCategory.disabled = false; // 소분류 활성화
                        } else {
                            lastCategory.innerHTML = '<option disabled selected value="">하위 카테고리 없음</option>';
                        }
                    })
